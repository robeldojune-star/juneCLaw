...
# Test parameters based on market practice: SL -6%, TP1 12%, TP2 17% (first+5), MH 3
print("Testing SL=-6%, TP1=12%, TP2=17%, MH=3 on recent days")
stock = '005930'
...
for d in dates:
    ...
    if not bars:
        print(f"  {d}: no bars")
        continue
    res = simulate_one_day(bars, stop_loss_pct=-6.0, take_profit_pct=12.0, take_profit_half_pct=17.0, max_holding_days=3)
    if res.get('ok'):
        all_nets.append(res['net_return_pct'])
        print(f"  {d}: net={res['net_return_pct']:.4f}%")
    else:
        print(f"  {d}: blocked: {res.get('blocking_conditions')}")
if all_nets:
    avg = sum(all_nets)/len(all_nets)
    print(f"\nAverage net return over {len(all_nets)} days: {avg:.4f}%")
else:
