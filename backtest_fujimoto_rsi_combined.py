...
def simulate_combined(bars, *, stop_loss_pct=-5.0, take_profit_pct=3.0, take_profit_half_pct=5.0, max_holding_days=3, fee_bps=23.0, slippage_bps=10.0, min_score=50, rsi_period=14, rsi_threshold=35):
    ...
    # align: first rsi_period-1 entries correspond to bars[0..period-2]? We'll map by index.
    # We'll compute rsi for each bar index i: if i < len(rsi_vals) then rsi_vals[i] else None
    in_position = False
    entry_price = 0.0
    entry_dt = None
    remaining = 0.0
...
def main():
    ...
    # test on first 2 stocks, last 3 days
    stock_codes = KOSPI_TOP_50[:2]
    end_date = datetime.now(KST).date()
    start_date = end_date - timedelta(days=2)  # last 3 days
    print(f"Testing combined Fujimoto (min_score=50) + RSI≤{35} on {len(stock_codes)} stocks from {start_date} to {end_date}")
    # Prefetch bars per stock per date
    bars_by_stock_date = {}
