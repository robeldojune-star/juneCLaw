#!/usr/bin/env python3
"""
Grid search for stop loss and take profit parameters based on market common practice:
- Stop loss: -5% to -8% (step 0.5%)
- Take profit first target: 10% to 15% (step 1%)
- Second target: first target + 5% (so total 15% to 20%)
- Max holding days: 2 to 4
- Uses re-entry logic as before.
Runs on first 10 stocks and last 5 days for speed.
"""
import sys
import json
import csv
...
def main():
    ...
    for sc in stock_codes:
        ...
        # fetch all bars for the date range in one go? We'll fetch per date for simplicity but cache.
        for d in dates:
    ...
    # Parameter grids
    stop_loss_options = [round(x, 1) for x in [-5.0, -5.5, -6.0, -6.5, -7.0, -7.5, -8.0]]
    tp_first_options = [round(x, 1) for x in [10.0, 11.0, 12.0, 13.0, 14.0, 15.0]]
    ...
    # Save results
    out_path = Path('/home/june/trading/reports/grid_search_market_practice.json')
