#!/usr/bin/env python3
"""
Backtest with stop loss -5% (instead of -1%) to see impact on returns.
Same as optimized script but with stop_loss_pct=-5.0.
"""
import sys
import json
import csv
...
def simulate_custom_swing_with_reentry(bars, *, stop_loss_pct=-5.0, take_profit_pct=3.0, take_profit_half_pct=5.0, max_holding_days=3, fee_bps=23.0, slippage_bps=10.0, min_score=60.0):
    if not bars:
...
def main():
    ...
    print("Parameters: stop loss -5%, take profit +3% -> +5%, max holding 3 days, fee 23 bps, slippage 10 bps")
    all_results = []
    ...
    out_path = Path('/home/june/trading/reports/fujimoto_126_backtest_custom_swing_with_reentry_stoploss_m5.json')
