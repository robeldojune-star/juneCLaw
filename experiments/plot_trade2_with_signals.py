...
from core.fujimoto_126_filter import evaluate_fujimoto_126
...
def fetch_bars(sb, stock_code, date_str):
    ...
    for row in rows:
        ...
        if dt.date() == datetime.strptime(date_str, '%Y-%m-%d').date():
            bars.append((dt, float(row['open']), float(row['high']), float(row['low']), float(row['close']), int(row['volume'] or 0)))
    bars.sort(key=lambda x: x[0])
    return bars

def make_svg(bars, entry_time_str, exit_time_str, entry_price, exit_price, title, signals=None):
    if not bars:
        return None
    # Determine min and max price for scaling
    prices = [p for _, _, _, _, p, _ in bars]  # close prices
    ...
    for i, (t, o, h, l, c, v) in enumerate(bars):
        ...
        y_c = y_pos(c)
        if i == 0:
            path_cmds.append(f'M {x:.2f},{y_c:.2f}')
        else:
            path_cmds.append(f'L {x:.2f},{y_c:.2f}')
    path = ' '.join(path_cmds)
    ...
    ex, ey = x_pos(entry_point[0]), y_pos(entry_point[4])  # close price
    xx, yx = x_pos(exit_point[0]), y_pos(exit_point[4])  # close price
    # Build SVG
  ...
  <polyline fill="none" stroke="blue" stroke-width="2" points="{path}"/>'''
    ...
    if signals:
        for i, (t, o, h, l, c, v) in enumerate(bars):
            if signals[i]:  # if signal is True at this point
                x = x_pos(t)
                y_low = y_pos(l)  # place marker slightly below low
                # Draw a small triangle pointing down
                svg += f'''
  <polygon points="{x-4},{y_low+4} {x+4},{y_low+4} {x},{y_low-4}" fill="orange" stroke="darkorange" stroke-width="1"/>'''
    # Entry and exit points
    svg += f'''
...
for label, trade in [('Best', best), ('Worst', worst)]:
    ...
    if not bars:
        ...
        continue
    # Compute signals for each point
    signals = []
    for i in range(len(bars)):
        window = bars[:i+1]
        # Convert window format to what evaluate_fujimoto_126 expects
        # Based on core/fujimoto_126_filter.py, it expects list of PriceBar objects
        # But for simplicity, let's reuse the logic from backtest
        # Actually, let's just use the same approach as in backtest_fujimoto_custom_swing.py
        from core.fujimoto_126_filter import PriceBar
        price_bars = []
        for (dt, o, h, l, c, v) in window:
        ...
        eval_result = evaluate_fujimoto_126(price_bars, min_score=60.0)
        signal = eval_result.get("signal", "")
        signals.append(signal == "HIGH_CONFIDENCE_CANDIDATE")
    svg = make_svg(bars,
                   trade['entry_time'],
                   trade['exit_time'],
                   trade['entry_price'],
    ...
    if svg:
        ...
        out_path = f"/tmp/{trade['stock_code']}_{trade['date']}_{label.lower()}_with_signals.svg"
