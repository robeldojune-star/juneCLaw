...
def simulate_one_day(bars, stop_loss_pct, take_profit_pct, take_profit_half_pct, max_holding_days):
    if not bars:
    ...
    while i < n:
        bar = bars[i]
        window = bars[: i + 1]
        eval_result = evaluate_fujimoto_126(window, min_score=60.0)
    ...
    if in_position and remaining > 0 and entry_price > 0:
        ...
        in_position = False
    if not trade_chunks:
        final_eval = evaluate_fujimoto_126(bars, min_score=60.0)
    ...
    for tc in trade_chunks:
        ...
        if entry > 0:
            ...
            cost_per_unit = ((23.0 + 10.0) / 100.0) * 2.0
...
# Test with one stock and one day
stock = '005930'
kst_date = datetime(2026, 5, 31).date()
bars = fetch_bars_kst_date(stock, kst_date)
print(f'Bars for {stock} on {kst_date}: {len(bars)}')
if len(bars) == 0:
    print('No bars, trying previous day')
    kst_date = datetime(2026, 5, 30).date()
    bars = fetch_bars_kst_date(stock, kst_date)
    print(f'Bars for {stock} on {kst_date}: {len(bars)}')

if bars:
    # Test a single parameter set
    res = simulate_one_day(bars, stop_loss_pct=-0.5, take_profit_pct=2.0, take_profit_half_pct=4.0, max_holding_days=3)
    print('Result:', res)
else:
    print('Still no bars')