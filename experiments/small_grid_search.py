#!/usr/bin/env python3
"""
Small grid search for swing strategy parameters on a single stock and a few days.
- Stop loss: [-0.3, -0.5, -0.7]
- Take profit first target: [1.5, 2.0, 2.5]
- Second target: first target + 2.0
- Max holding days: [2, 3]
- Uses one stock (005930) and the last 3 days.
"""
import sys
import json
...
def main():
    ...
    start_date = end_date - timedelta(days=2)  # last 3 days inclusive
    dates = [start_date + timedelta(days=i) for i in range((end_date - start_date).days + 1)]
    print(f"Testing {stock} on dates: {[d.isoformat() for d in dates]}")
    
    # Prefetch bars for each date
    bars_by_date = {}
    for d in dates:
    ...
    for sl in stop_loss_options:
        for tp in tp_first_options:
            for mh in max_hold_options:
                ...
                print(f"SL={sl:.2f}%, TP1={tp:.2f}%, TP2={tp2:.2f}%, MH={mh} -> Avg={avg_ret:.4f}%, WR={win_rate:.2f}%, Trades={len(all_trade_nets)}/{total_days}")
    ...
    if results:
        ...
        print(f"Average net return: {best['avg_return']:.4f}%")
        print(f"Win rate: {best['win_rate']:.2f}%")
        # Save results
        out_path = Path('/home/june/trading/reports/small_grid_search.json')
