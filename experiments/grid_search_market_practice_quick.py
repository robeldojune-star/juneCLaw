#!/usr/bin/env python3
"""
Quick grid search for stop loss and take profit based on market practice:
- Stop loss: -5% to -8% (step 1%)
- Take profit first target: 10% to 15% (step 2%)
- Second target: first target + 5%
- Max holding days: 2 to 4
- Uses re-entry logic.
Runs on first 5 stocks and last 3 days.
"""
import sys
import json
import csv
...
def main():
    ...
    # Parameter grids
    stop_loss_options = [-5.0, -6.0, -7.0, -8.0]
    tp_first_options = [10.0, 12.0, 14.0]  # step 2% for speed
    ...
    # Save results
    out_path = Path('/home/june/trading/reports/grid_search_market_practice_quick.json')
