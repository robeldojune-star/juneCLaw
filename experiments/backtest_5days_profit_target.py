...
trade_details = []  # list of dicts for each trade
...
if trades:
    ...
    for td in trade_details:
        print(f"  Date {td['date']} Entry {td['entry_time']} @ {td['entry_price']:.0f} -> Exit {td['exit_time']} @ {td['exit_price']:.0f} Profit {td['profit_pct']:.2f}%")
else:
