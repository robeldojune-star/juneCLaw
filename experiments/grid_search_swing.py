#!/usr/bin/env python3
"""
Grid search for stop loss and take profit parameters for the swing strategy with re-entry.
...
Runs on a subset of stocks (first 20) for speed, then reports best parameters.
"""
import sys
import json
import csv
...
def main():
    ...
    # Use subset for speed: first 20 stocks
    stock_codes = KOSPI_TOP_50[:20]
    end_date = datetime.now(KST).date()
    start_date = end_date - timedelta(days=13)  # approx 2 weeks
    print(f"Grid search on {len(stock_codes)} stocks from {start_date} to {end_date}")
    ...
    # Save results
    out_path = Path('/home/june/trading/reports/grid_search_swing.json')
