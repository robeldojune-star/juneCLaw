#!/usr/bin/env python3
"""
Parameter optimization for Fujimoto 1-2-6 strategy with custom rules and re-entry.
Grid search over stop loss, take profit levels, and max holding days.
Runs on a subset of stocks (first N) for speed, then evaluates on full set.
"""
import sys
import json
import csv
    ...
    if successful:
        ...
        if returns:
            ...
            return {
                ...
                'results': all_results  # optionally keep for debugging
            }
        else:
...
def main():
    ...
    stop_loss_options = [-0.5, -1.0, -1.5]  # percent
    take_profit_first_options = [2.0, 3.0, 4.0]  # first target percent
    take_profit_second_options = [4.0, 5.0, 6.0]  # second target percent (for remaining half)
    max_holding_options = [2, 3, 4]  # days
    
    # Use a subset for quick evaluation (first 10 stocks)
    subset_stocks = KOSPI_TOP_50[:10]
    ...
    # Now run the best parameters on the full KOSPI Top 50
    print("\n=== Running best parameters on FULL KOSPI Top 50 ===")
    start_time = time.time()
    ...
    # Save results
    out_path = Path('/home/june/trading/reports/fujimoto_126_param_optimization.json')
