#!/usr/bin/env python3
"""
Quick parameter optimization runner for Fujimoto 1-2-6 strategy.
Runs the backtest with a reduced parameter set and shorter date range for speed.
"""
import sys
import itertools
import subprocess
...
def run_backtest_with_params(stop_loss_pct, take_profit_half_pct, take_profit_pct):
    ...
    # Read the original script (we'll use the custom swing script)
    ...
    pattern = r'def simulate_custom_swing\(bars, \*, stop_loss_pct=-1\.0, take_profit_pct=3\.0, take_profit_half_pct=5\.0, max_holding_days=3, fee_bps=23\.0, slippage_bps=10\.0, min_score=60\.0\):'
    replacement = f'def simulate_custom_swing(bars, *, stop_loss_pct={stop_loss_pct}, take_profit_pct={take_profit_pct}, take_profit_half_pct={take_profit_half_pct}, max_holding_days=3, fee_bps=23.0, slippage_bps=10.0, min_score=60.0):'
    modified_content = re.sub(pattern, replacement, content)

    # Also modify the main function to reduce the date range and stock list for speed
    # We'll replace the lines that set stock_codes, start_date, and end_date
    # We'll look for the line that sets stock_codes and replace it with a smaller list
    # and then adjust the date range to last 3 days.
    # We'll do this by replacing a block of lines in the main function.

    # First, let's find the main function and then replace the relevant lines.
    # We'll do a simpler approach: replace the entire main function with a shortened version?
    # Instead, we'll replace just the two lines we need.

    # We'll replace the line that sets stock_codes to use only the first 5 stocks.
    # And we'll replace the line that sets start_date to be end_date - 2 days (3 days total).

    # We'll do two replacements:

    # 1. Replace stock_codes = KOSPI_TOP_50[:20] with stock_codes = KOSPI_TOP_50[:5]
    ...
    # 2. Replace start_date = end_date - timedelta(days=13) with start_date = end_date - timedelta(days=2)
    modified_content = re.sub(
...
def main():
    ...
    print("Starting Fujimoto 1-2-6 strategy parameter optimization (quick test)...")
    ...
    output_file = output_dir / "fujimoto_optimization_results_quick.csv"
