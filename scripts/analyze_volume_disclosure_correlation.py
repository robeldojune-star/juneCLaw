...
def main():
    ...
    print("Fetching daily prices (all rows, then filter in memory)...")
    # Fetch all daily_prices rows (we have about 1000 rows, which is fine)
    daily_prices = sb.get('daily_prices')
    print(f"Fetched {len(daily_prices)} daily price rows.")
    
    # Get the set of stock codes we care about (from disclosures)
    stock_codes = set(stock for stock, _ in disclosure_counts.keys())
    print(f"Filtering daily prices for {len(stock_codes)} stocks from disclosures.")
    
    # Organize daily prices by stock and date, but only for stocks we care about
    prices_by_stock = defaultdict(dict)  # stock_code -> {date: {volume, ...}}
    for p in daily_prices:
        stock = p['stock_code']
        if stock not in stock_codes:
            continue
        date_str = p['date']  # YYYY-MM-DD
        prices_by_stock[stock][date_str] = p
    
    print(f"After filtering, we have data for {len(prices_by_stock)} stocks.")
