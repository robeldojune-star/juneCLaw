#!/usr/bin/env python3
"""
Quick test of stop loss -5% using the re-entry swing strategy.
Keeps other parameters: TP1 +3%, TP2 +5%, max holding 3 days.
Uses same optimization approach: fetch bars per stock once, group by date.
"""
import sys
import json
import csv
...
def main():
    ...
    # test subset of first 10 stocks for speed
    stock_codes = KOSPI_TOP_50[:10]
    end_date = datetime.now(KST).date()
    start_date = end_date - timedelta(days=13)  # approx 2 weeks as before
    print(f"Testing stop loss -5% from {start_date} to {end_date} (KST) on {len(stock_codes)} stocks")
    all_results = []
    ...
    for idx, stock_code in enumerate(stock_codes, 1):
        ...
        while current <= end_date:
            ...
            current += timedelta(days=1)
        print(f"  Completed {stock_code} in {time.time()-stock_start:.1f}s")
    # Summary
    print("\n=== Summary ===")
    ...
    out_path = Path('/home/june/trading/reports/stoploss_minus5_test.json')
