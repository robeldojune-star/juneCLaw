#!/usr/bin/env python3
"""
Efficient grid search for swing strategy parameters.
- Pre-fetch all intraday bars for each stock in the date range once.
- Group by date.
- Loop over parameter grid and simulate using pre-grouped data.
- Stop loss range: -0.3 to -0.8 (step 0.1)
- TP1 range: 1.5 to 3.0 (step 0.25)
- TP2 = TP1 + 2.0
- Max holding days: 2 to 4
- Uses first 5 stocks and last 5 days for quick test.
"""
import sys
import json
import csv
...
def fetch_bars_date_range(sb, stock_code, start_date, end_date):
    """Fetch all 1-minute bars for stock_code between start_date and end_date inclusive."""
    # Use gte and lt on timestamp (end_date + 1 day)
    start_str = start_date.isoformat()
    end_str = (end_date + timedelta(days=1)).isoformat()  # exclusive upper bound
...
def main():
    ...
    # Use first 5 stocks for speed
    stock_codes = KOSPI_TOP_50[:5]
    end_date = datetime.now(KST).date()
    start_date = end_date - timedelta(days=4)  # last 5 days inclusive
    print(f"Parameters will be tested on {len(stock_codes)} stocks from {start_date} to {end_date}")
    # Pre-fetch bars for each stock
    print("Fetching intraday bars...")
    bars_by_stock_date = {}  # stock_code -> {date: [bars]}
    fetch_start = time.time()
    for sc in stock_codes:
        print(f"  Fetching {sc}...")
        bars = fetch_bars_date_range(sb, sc, start_date, end_date)
        # group by date
    ...
    # Show top 5
    sorted_res = sorted(results, key=lambda x: x['avg_return'], reverse=True)[:5]
    ...
    # Save results
    out_path = Path('/home/june/trading/reports/grid_search_swing_efficient.json')
